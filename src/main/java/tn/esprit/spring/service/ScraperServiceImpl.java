...
public class ScraperServiceImpl  {
    ...
    @Autowired
    private MaisonVacanceRepository maisonVacanceRepository;


    public Set<MaisonVacance> getMaisonVcance(MaisonVacance maisonVacance) {
        //Using a set here to only store unique elements
        Set<MaisonVacance> maisonVacances = new HashSet<>();
        //Traversing through the urls
        for (String url: urls) {

            if (url.contains("bnb.tn")) {
                //modifying the URL to include the region name
                String urlWithRegion = url ; //replace "region-name" with the actual region name
                //method to extract data from Ikman.lk
                extractDataFromBnb(maisonVacances, urlWithRegion);
            }

        }

        return maisonVacances;
    }


    private void extractDataFromBnb(Set<MaisonVacance> maisonVacances, String url) {
        try {
            //loading the HTML to a Document Object
            Document document = Jsoup.connect(url).get();
            //Selecting the element which contains the ad list
            Elements liste = document.select("article");
            for (Element row : liste) {
                MaisonVacance maisonVacance = new MaisonVacance();

                // Extract region name from first <td> in row
                String maisonName = row.select("h3.property-row-title").text();
                maisonVacance.setNameMaison(maisonName);
                String region = row.select("div.property-row-location").text();

                // Extract population and surface area from remaining <td> elements in row


                maisonVacances.add(maisonVacance);
                maisonVacanceRepository.save(maisonVacance);
            }
        } catch (IOException ex) {
            ex.printStackTrace();
        }
    ...
    public void scrapeMaisonVacances() throws IOException {
        List<MaisonVacance> maisonVacances = new ArrayList<>();
        ...
        while (pageNumber <= 53 ) {
            String url = "";
            if (pageNumber == 1){
                 url = "https://www.bnb.tn/contract/location-de-vacances";}
            else
                 url = "https://www.bnb.tn/contract/location-de-vacances/page/" + pageNumber;
            Document doc = Jsoup.connect(url).get();
            Elements maisonElements = doc.select("article");

            for (Element maisonElement : maisonElements) {
                MaisonVacance maisonVacance = new MaisonVacance();
                Elements propretyHouse = maisonElement.select(".property-row-meta");
                HashMap<String , String> ss = new HashMap<>();
                ss.put("Lits ","");
                ss.put("Surface " , "");
                ss.put("Garages ", "");
                ss.put("Salles de bain " , "");
                for (Element prop : propretyHouse){


                    Elements pp = prop.select(".field-item");
                    for(Element p : pp){
                        System.out.println("**********************");
                        ...
                        if (matcher.find()) {
                            ...
                            ss.put(p.text().split("\\d+")[0] , matcher.group());
                ...
                maisonVacance.setNameMaison(maisonElement.select("h3.property-row-title").text());
                maisonVacance.setPrix(maisonElement.select(".property-row-price").text().split(" ")[0]);
                maisonVacance.setRegion(maisonElement.select(".property-row-location").text().split("à ")[1]);

                maisonVacance.setNbrLits( ss.get("Lits ") !="" ? Integer.parseInt(ss.get("Lits ")) :0 );
                maisonVacance.setSurface(ss.get("Surface ")  !="" ? Integer.parseInt(ss.get("Surface ")) :0);
                maisonVacance.setSalleDeBain(ss.get("Salles de bain ")  !="" ? Integer.parseInt(ss.get("Salles de bain ")) :0);
                maisonVacance.setGarage(ss.get("Garages ")  !="" ? Integer.parseInt(ss.get("Garages ")) :0);


                //maisonVacance.setSurface(maisonElement.select(".field-item .label:contains('Surface')").text());
               // maisonVacance.setSurface(maisonElement.select(".card-body > .icon-surface").text());
